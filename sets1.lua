...
getgenv().PredictionVelocity = 8.009 -- The speed of the PredictMovement feature 
...
local AimlockTarget;
getgenv().okokokUniversalAimbotLoaded = true
    ...
    if workspace and workspace:FindFirstChild"Teams" then 
        if getgenv().TeamCheck == true then
            if #workspace.Teams:GetChildren() == 0 then 
                ...
                SeparateNotify("okokok", "TeamCheck set to: "..tostring(getgenv().TeamCheck).." because there are no teams!", "", 3)
            end
        end
    end
end
		...
		if input.KeyCode == Enum.KeyCode.P then 
			pcall(function()
				...
				if Target ~= nil then 
					...
					Notify("okokok", "Aimlock Target: "..tostring(AimlockTarget), "", 3)
				end
			end)
		end
		if input.KeyCode == Enum.KeyCode[AimlockToggleKey] then 
			Aimlock = not Aimlock
			Notify("okokok", "Aimlock: "..tostring(Aimlock), "", 3)
		end
	end
end)
